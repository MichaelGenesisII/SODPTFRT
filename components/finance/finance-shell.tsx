...
import { signOutFinance } from "@/app/finance/actions";
...
const SIDEBAR_KEY = "sod-finance-sidebar-open";
...
export function FinanceShell({
  ...
  const name = financeDisplayName(profile);
  const [mobileOpen, setMobileOpen] = useState(false);
  const [desktopOpen, setDesktopOpen] = useState(true);
  const [confirmSignOut, setConfirmSignOut] = useState(false);
  const current =
    nav.find((item) => pathActive(item.href, pathname))?.label ?? "Finance";
  ...
  return (
    <div className="relative flex min-h-svh flex-col bg-mist text-ink lg:flex-row">
        ...
        <header className="sticky top-0 z-30 hidden h-[4.75rem] items-center justify-between gap-4 border-b border-stone/80 bg-mist/90 px-6 backdrop-blur-xl lg:flex lg:px-10">
          <div className="flex min-w-0 items-center gap-3">
            ...
            <div>
              <p className="text-[0.65rem] font-medium uppercase tracking-[0.16em] text-ink/40">
                Finance <span className="mx-2 text-stone">/</span>
          ...
          <div className="flex items-center gap-3 text-right">
            <div className="min-w-0">
              ...
              <p className="mt-0.5 text-xs text-ink/45">Finance account</p>
            </div>
            <StaffAvatar
              name={name}
      ...
      <SignOutConfirmModal
        ...
        signOut={signOutFinance}
        portalLabel="the Finance portal"
      />
    </div>
  );
}
...
function RateIcon({ className }: { className?: string }) {
  return (
    <svg className={className} viewBox="0 0 24 24" fill="none" aria-hidden>
      ...
      <path
        d="M12 8v8M9.5 10.5c.5-.8 1.4-1.2 2.5-1.2 1.4 0 2.5.7 2.5 1.9s-1.1 1.9-2.5 1.9h-1c-1.4 0-2.5.7-2.5 1.9 0 1.2 1.1 1.9 2.5 1.9 1.1 0 2-.4 2.5-1.2"
        stroke="currentColor"
        strokeWidth="1.5"
        strokeLinecap="round"
      />
...
function TeachersIcon({ className }: { className?: string }) {
  return (
    <svg className={className} viewBox="0 0 24 24" fill="none" aria-hidden>
      ...
      <path
        d="M4.5 18c1.2-2 3-3 4.5-3s3.3 1 4.5 3M13.5 18c.7-1.2 1.8-1.9 3-1.9 1 0 1.8.4 2.5 1.1"
        stroke="currentColor"
        strokeWidth="1.5"
        strokeLinecap="round"
      />
