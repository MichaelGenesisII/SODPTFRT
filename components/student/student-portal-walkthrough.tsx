...
import {
  STUDENT_PORTAL_TOUR_QUEST_COUNT,
...
type StudentPortalWalkthroughProps = {
  open: boolean;
  stepIndex: number;
  pathname: string;
  hash: string;
  ...
  onNext: () => void;
  onBack: () => void;
  onRequestHomeSection: (section: StudentHomeSection) => void;
  onRequestTab: (payload: StudentTourTabPayload) => void;
};
...
export function StudentPortalWalkthroughTrigger({
  onClick,
}: {
  onClick: () => void;
}) {
...
export function StudentPortalWalkthrough({
  open,
  stepIndex,
  pathname,
  hash,
  ...
  onNext,
  onBack,
  onRequestHomeSection,
  onRequestTab,
}: StudentPortalWalkthroughProps) {
  const driverRef = useRef<Driver | null>(null);
  const endingRef = useRef(false);
  const navStartedRef = useRef(0);
  const stepKeyRef = useRef(stepIndex);
  const onCloseRef = useRef(onClose);
  const onFinishRef = useRef(onFinish);
  const onRequestHomeSectionRef = useRef(onRequestHomeSection);
  ...
  const [, setNavClock] = useState(0);

  const step = STUDENT_PORTAL_TOUR_STEPS[stepIndex] ?? STUDENT_PORTAL_TOUR_STEPS[0]!;
  const isLast = stepIndex >= STUDENT_PORTAL_TOUR_STEPS.length - 1;

  if (stepKeyRef.current !== stepIndex) {
    stepKeyRef.current = stepIndex;
    navStartedRef.current = Date.now();
  } else if (open && navStartedRef.current === 0) {
    navStartedRef.current = Date.now();
  }
  ...
  const onStepPage = tourHrefReady(
  ...
  const navElapsed =
    open && navStartedRef.current
      ? Date.now() - navStartedRef.current
      : 0;
  const navTimedOut = navElapsed >= TOUR_NAV_MAX_MS;
  const navigating = Boolean(step.target) && !onStepPage && !navTimedOut;
  const canContinue = !step.target || onStepPage || navTimedOut;

  useEffect(() => {
    if (!open) navStartedRef.current = 0;
  }, [open]);

  useEffect(() => {
    if (!open) return;
    const id = window.setInterval(() => setNavClock((c) => c + 1), 200);
  ...
  useEffect(() => {
    ...
    onRequestHomeSectionRef.current = onRequestHomeSection;
    onRequestTabRef.current = onRequestTab;
  }, [onClose, onFinish, onRequestHomeSection, onRequestTab]);
  ...
  useEffect(() => {
    ...
    const current = STUDENT_PORTAL_TOUR_STEPS[stepIndex];
    if (!current) return;

    const currentLive = readLiveTourLocation();
    if (
      !tourHrefReady(
        currentLive.pathname,
        currentLive.hash,
        tourStepPath(current),
        currentLive.search || search,
      )
    ) {
      teardownTourDriver(driverRef.current);
      driverRef.current = null;
    ...
    if (current.homeSection) onRequestHomeSectionRef.current(current.homeSection);
    if (current.tourTab) onRequestTabRef.current(current.tourTab);
    ...
    const run = () => {
      ...
      const selector = resolveTourTarget(
        current.target,
        current.fallbackTargets,
      );
      if (!selector && attempts < TOUR_TARGET_MAX_ATTEMPTS) {
        attempts += 1;
        timer = window.setTimeout(run, TOUR_TARGET_POLL_MS);
        return;
      }
      if (!selector) setSpotlightMissing(true);
      if (!driverRef.current) driverRef.current = ensureDriver();
      highlightTourTarget(driverRef.current, selector, setHudPlacement);
    ...
    timer = window.setTimeout(
      ...
      needsSidebar
        ? 420
        : current.tourTab || current.homeSection
          ? 280
          : 80,
    );
    return () => window.clearTimeout(timer);
  }, [open, stepIndex, pathname, hash, search]);
  ...
  const title = stepIndex === 0 ? `Hey ${greetingName}` : step.title;
  const dimOnly = !step.target || !onStepPage || navigating;
  const trackSpotlight = Boolean(step.target) && onStepPage && !navigating;
  ...
  return (
    <>
      ...
      <DavidTourHud
        ...
        titleId="student-portal-tour-title"
      >
        <div className="flex items-center justify-between gap-3">
                <span className="inline-flex items-center gap-2 bg-pine px-2.5 py-1 text-[0.65rem] font-semibold uppercase tracking-[0.14em] text-mist">
                  <span
                    ...
                    aria-hidden
                  />
                  Quest {step.quest} / {STUDENT_PORTAL_TOUR_QUEST_COUNT}
                </span>
                <button
                  type="button"
              ...
              {spotlightMissing && step.missingNote ? (
                <p className="mt-2 text-sm leading-relaxed text-ink/55 italic">
                  {step.missingNote}
                </p>
              ) : null}
              ...
              <div
                ...
                aria-hidden
              >
                {STUDENT_PORTAL_TOUR_STEPS.filter((s) => s.quest === step.quest).map(
                  (s) => {
                    const absoluteIndex = STUDENT_PORTAL_TOUR_STEPS.findIndex(
                      (x) => x.id === s.id,
                    );
                    return (
                      <span
              ...
              <p className="mt-2 text-[0.65rem] tabular-nums text-ink/35">
                Stop {stepIndex + 1} of {STUDENT_PORTAL_TOUR_STEPS.length}
              </p>
