...
/** Notify a teacher they have been assigned to teach a class. */
export function buildClassTeacherAssignmentEmail(
  input: ClassTeacherAssignmentInput,
) {
  const firstName = input.firstName.trim() || "colleague";
  ...
  const portalUrl = escapeHtml(input.teacherPortalUrl.trim());
  ...
  const assignedBy = input.assignedByName?.trim()
    ? escapeHtml(input.assignedByName.trim())
    : "";
  ...
  const subject = `${firstName}, you are scheduled to teach: ${input.classTitle.trim()}`;

  const textParts = [
    `Hello ${firstName},`,
    ``,
    assignedBy
      ? `${input.assignedByName!.trim()} has assigned you to teach a School of Disciples class.`
      : `You have been assigned to teach a School of Disciples class.`,
    ``,
  ...
  textParts.push(
    ``,
    `Open your teacher portal for the schedule, register, and delivery confirmation:`,
    input.teacherPortalUrl,
  );
  ...
  textParts.push(
    ...
    `© ${year} School of Disciples · ${input.siteUrl}`,
  );
        ...
        <table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="max-width:580px;">
          ...
          <tr>
            <td style="background:#f7faf8;padding:22px 22px 8px;border-left:1px solid #c5d6cc;border-right:1px solid #c5d6cc;font-family:Arial,Helvetica,sans-serif;font-size:15px;line-height:1.65;color:rgba(20,36,28,0.78);">
              <p style="margin:0;">
                ${
                  assignedBy
                    ? `${assignedBy} assigned you this class. Open your teacher portal for the schedule, register, and to confirm delivery.`
                    : `Open your teacher portal for the schedule, register, and to confirm delivery.`
                }
              </p>
              ${
                notes
          ...
          ${
            ...
            <td style="background:#ffffff;padding:8px 22px 22px;border:1px solid #c5d6cc;border-top:0;">
              ...
              : `<tr>
            <td style="background:#ffffff;padding:0 0 1px;border:1px solid #c5d6cc;border-top:0;"></td>
          </tr>`
          }

          <tr>
            <td style="padding:18px 8px 0;font-family:Arial,Helvetica,sans-serif;font-size:12px;line-height:1.55;color:rgba(20,36,28,0.45);">
              <p style="margin:0;">Questions? <a href="${supportUrl}" style="color:#3d6b58;">Portal support</a></p>
              <p style="margin:10px 0 0;">This is an automated message. Please do not reply to this email.</p>
              <p style="margin:8px 0 0;">© ${year} School of Disciples · <a href="${siteUrl}" style="color:#3d6b58;">${siteUrl.replace(/^https?:\/\//, "")}</a></p>
            </td>
          </tr>
