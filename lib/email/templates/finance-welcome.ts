export type FinanceWelcomeInput = {
  fullName: string;
  email: string;
  temporaryPassword: string;
  inviterName: string;
  financeLoginUrl: string;
...
/** Welcome for a newly invited Finance Admin — calm, no operational detail. */
export function buildFinanceWelcomeEmail(input: FinanceWelcomeInput) {
  ...
  const loginUrl = escapeHtml(input.financeLoginUrl);
  const supportUrl = escapeHtml(input.portalSupportUrl);
  const siteUrl = escapeHtml(input.siteUrl);
  const year = new Date().getFullYear();

  const subject = "Your School of Disciples teacher finance access";

  const text = [
    `Hello ${firstName},`,
    "",
    `${input.inviterName.trim() || "The national desk"} has invited you to teacher finance on the School of Disciples portal.`,
    "",
    "Sign in:",
    input.financeLoginUrl,
    "",
    ...
    <tr><td align="center">
      <table role="presentation" width="100%" style="max-width:560px;background:#ffffff;border:1px solid #d5ddd6;">
        <tr><td style="padding:28px 28px 8px;background:#14352c;">
          <p style="margin:0;font-size:11px;letter-spacing:0.16em;text-transform:uppercase;color:#95bfa8;">Teacher finance</p>
          <h1 style="margin:12px 0 0;font-size:26px;line-height:1.15;color:#f4f7f5;font-weight:normal;">Welcome, ${name}</h1>
        </td></tr>
        <tr><td style="padding:24px 28px;">
          <p style="margin:0 0 18px;font-size:16px;line-height:1.55;">${inviter} has invited you to teacher finance on the School of Disciples portal.</p>
          ...
          <p style="margin:22px 0 0;"><a href="${loginUrl}" style="display:inline-block;padding:12px 20px;background:#14352c;color:#f4f7f5;text-decoration:none;font-size:14px;">Open sign-in</a></p>
          <p style="margin:18px 0 0;font-size:13px;line-height:1.5;color:#5a655e;">Please change your password after first sign-in. Need help? <a href="${supportUrl}" style="color:#14352c;">Portal support</a>.</p>
        </td></tr>
