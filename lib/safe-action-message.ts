...
const TECHNICAL =
  /body exceeded|bodySizeLimit|server action|ECONNREFUSED|ENOTFOUND|ETIMEDOUT|ECONNRESET|socket hang up|getaddrinfo|SMTP|nodemailer|Resend|resend\.com|Postgrest|PGRST|supabase\.co|supabase\/|relation ["']|column ["'].*does not exist|violates (foreign key|unique|check)|permission denied for|stack trace|at\s+\S+\s+\(|\.sql\b|row level security|\brls\b|service[_ ]?role|security definer|postgres|uuid_generate|could not find the table|schema cache|zoom api|api\.zoom\.us|"code":\d{4}/i;
